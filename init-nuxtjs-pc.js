...
export async function initNuxtJSPC({ projectName }) {
  console.log(`init NuxtJS PC: ${Conf.NUXT_PC}`)

  // 下载项目模版
  await clone(`direct:${Conf.NUXT_PC}#master`, projectName, { clone: true })
  const pwd = shell.pwd()

  // 清理文件
  const deleteDir = ['.git', 'README.md', 'doc']
